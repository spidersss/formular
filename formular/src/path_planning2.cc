...
typedef std::vector<cv::Point> newpoint;
newpoint path_planning(newpoint side_1, newpoint side_2);
/**************/
...
void cloud_cb(const sensor_msgs::PointCloud2 &cloud_msg)
{
	...
	trajectory = path_planning(side_1, side_2);
	//**最终路径绘制**rviz 版本//
	...
	for (int i = trajectory.size() - 1; i >= 1; --i)
	{
		...
		this_pose_stamped.pose.position.x = trajectory[i].x; 
		this_pose_stamped.pose.position.y = trajectory[i].y; 
		geometry_msgs::Quaternion goal_quat = tf::createQuaternionMsgFromYaw(atan2(trajectory[i].y,trajectory[i].x));
...
newpoint path_planning(newpoint side_1,newpoint side_2)
{
	...
	int success = 0;
	int search_count = 0;
	while (success == 0 && search_count < 100)
	{
	...
	if(search_count > 90) cout<<"本次失败!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<endl;
	finish = clock();  //结束计时
	...
	cout << "程序运行结束！" << endl;
	vector<Point>path_spots;
	for (int i = 0; i != path_x.size(); ++i)
	{
		double x = path_x[i];
		double y = path_y[i];
		path_spots.push_back(Point(x, y));
	}
	return path_spots;
}
